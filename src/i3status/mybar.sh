...
myvpn_on() {
  ...
  echo -n ",{"
  echo -n "\"name\":\"id_vpn\","
  echo -n "\"full_text\":\" ${icon} VPN: $(ifconfig tun0 | grep -w inet | awk '{print $2}') \","
  echo -n "\"background\":\"$bg\","
  common
  echo -n "},"
}
...
myip_local() {
  ...
  echo -n "\"full_text\":\"  Local IP: $(ip route get 1 | sed -n 's/.*src \([0-9.]\+\).*/\1/p') \","
  echo -n "\"background\":\"$bg\","
  common
  echo -n "},"
}
...
disk_usage() {
  ...
  echo -n "\"full_text\":\"  Disk: $(~/.config/i3status/disk.py)%\","
  echo -n "\"background\":\"$bg\","
  common
  echo -n "}"
}
...
memory() {
  ...
  echo -n "\"full_text\":\"  RAM: $(~/.config/i3status/memory.py)%\","
...
cpu_usage() {
  ...
  echo -n "\"full_text\":\"  CPU: $(~/.config/i3status/cpu.py)% \","
  echo -n "\"background\":\"#3949AB\","
  common
  echo -n "},"
}
...
myupdates() {
  ...
  echo -n "\"full_text\":\"  Updates: $(~/.config/i3status/updates.sh) \","
  echo -n "\"color\":\"#000000\","
  echo -n "\"background\":\"$bg\","
  common
  echo -n "},"
...
do
  ...
  # {"name":"id_vpn","button":1,"modifiers":["Mod4"],"x":2982,"y":9,"relative_x":67,"relative_y":9,"width":95,"height":22}

  # VPN click
  if [[ $line == *"name"*"id_vpn"* ]]; then
    terminator -e ~/.config/i3status/click_vpn.sh &
  ...
  # TIME
    ...
    terminator -e ~/.config/i3status/click_time.sh &

  # VOLUME
  elif [[ $line == *"name"*"id_volume"* ]]; then
    terminator -e alsamixer &
